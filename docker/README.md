...
a docker containers for *hello-world-api*!
...
```bash
make docker-gen IMAGE=hello-world-api VERSION=latest
```
...
```bash
make clean-docker IMAGE=hello-world-api VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *hello-world-api* uses
...
```bash
make docker-debug IMAGE=hello-world-api-debug VERSION=latest
```
...
```bash
docker run hello-world-api-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it hello-world-api-debug bash
```
...
```bash
make clean-docker IMAGE=hello-world-api-debug VERSION=latest
```
