...
.PHONY: install
## `install`: Fetch dependencies needed to run `hello-world-api`
install:
...
.PHONY: run
## :
## `run`: Run `hello-world-api` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `hello-world-api` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `hello-world-api`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `hello-world-api`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
